...
async function handleRegister(agent) {
  const userId = agent.originalRequest.payload.data.source.userId;
  console.log('userId', userId);
  const { bloodgroup, mobile, province } = agent.parameters;
  const body = {
    dbName,
    lineUserId: userId,
    mobile,
  ...
  await request.post({
    uri: `https://asia-northeast1-blood-plus-fchltb.cloudfunctions.net/api/register`,
    headers: {
  ...
  client.linkRichMenuToUser(userId, 'richmenu-da5fe8ea9aac345139111c5e1d16e83b');
...
async function handleRequest(agent) {
  ...
  const related = await request.post({
    uri: `https://asia-northeast1-blood-plus-fchltb.cloudfunctions.net/api/request`,
    headers: {
      "content-type": "application/json",
    },
    body,
  ...
  const message = {
    type: 'text',
    text: `${profile.displayName} ขอบริจาคเลือดกรุ๊ป ${bloodgroup} จังหวัด ${province} ติดต่อ ${mobile}`
  };
  let userIds = [];
  related.forEach(docInfo => {
    // if (docInfo.lineUserId !== userId) {
      userIds.push(docInfo.lineUserId);
    // }
  });
  client.multicast(userIds, message);

  agent.add('บันทึกข้อมูลสำเร็จแล้ว ระบบจะทำการติดต่อกลับโดยเร็วที่สุด');
}
